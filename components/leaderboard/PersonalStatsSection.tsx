...
function StatsLineChart({
  ...
  const chartPoints = points.map((p) => {
    ...
    const rawRatio = yRange > 0 ? (p.y - minVal) / yRange : 0.5
...
export default function PersonalStatsSection() {
  ...
  const weeklyFillers = useMemo(() => {
    ...
    // week identity: the Sunday that starts the week. sortKey (YYYY-MM-DD) keeps
    // weeks chronological and avoids collisions across months/years, while label
    // stays a compact MM/DD.
    const getSunday = (dateStr: string): { sortKey: string; label: string } => {
      const d = new Date(dateStr)
      if (Number.isNaN(d.getTime())) return { sortKey: '', label: 'Week' }
      const sunday = new Date(d)
      sunday.setDate(d.getDate() - d.getDay())
      const year = sunday.getFullYear()
      const month = String(sunday.getMonth() + 1).padStart(2, '0')
      const date = String(sunday.getDate()).padStart(2, '0')
      return { sortKey: `${year}-${month}-${date}`, label: `${month}/${date}` }
    }

    const weeks: Record<string, { label: string; totalFillers: number; count: number }> = {}
    for (const run of speedRuns) {
      const { sortKey, label } = getSunday(run.date)
      if (!weeks[sortKey]) weeks[sortKey] = { label, totalFillers: 0, count: 0 }
      weeks[sortKey].totalFillers += run.fillerCount
      weeks[sortKey].count += 1
    }
  ...
  return (
    <div className="personal-stats-section flex flex-col gap-5">
      ...
      <p className="stats-page-subtitle text-center">
